...
function UceGrade({data}){
  ...
  let { uneb_uce,theme, paper_mode, selected_clas } = data

  let perpage = paper_mode==='portrait'?45:40
  let firstpage = paper_mode==='portrait'?38:33
  let reports = chunkArray(uneb_uce?uneb_uce.UNEB_UCE:[], perpage, firstpage);
  data.info = { ...data.school_info, ...uneb_uce?.SCHOOL_INFO }
  //
  console.log('check info',data.info);
          ...
          <View style={styles.container}>
            <View style={styles.blurBorder}>
              ...
              <>
                ...
                <Text style={{ marginBottom:3,fontWeight:'bold', textAlign:'center', color:theme_bg, fontSize:14}}>{data?.info?.EXAM||selected_clas} - {data?.info?.YEAR}</Text> 
